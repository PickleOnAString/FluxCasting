...
import net.picklestring.flux_weavers.registries.BlockRegistry;
		...
		if (!context.getWorld().isClient) {
			if (context.getPlayer().isSneaking() && !context.getWorld().getBlockState(context.getBlockPos()).isOf(BlockRegistry.RUNE_TABLE)) {
				...
				if (!((RuneTableEntity) context.getWorld().getBlockEntity(context.getBlockPos())).isEmpty()) {
					player.sendMessage(Text.translatable("item.flux_weavers.spell_holder.table_not_empty"));
				...
				for (int i = 0; i < player.getInventory().size(); i++) {
					...
					if (!player.getInventory().containsAny(Collections.singleton(spell.get(i).getItem()))) {
						hasItems = false;
						player.sendMessage(Text.translatable("item.flux_weavers.spell_holder.missing_rune", Text.translatable(spell.get(i).getItem().getTranslationKey())));
						break;
					}
				}
