...
public class ProjectileDestinationRune extends RuneItem {
	public ProjectileDestinationRune(Settings settings) {
		super(
			settings,
			null,
			new Type[]{
	...
	public Object[] getValue(DefaultedList<ItemStack> inventory, int runeIndex, PlayerEntity caster, Vec3d pos, World world, CastingContext context) {
		if (!context.extraContext.containsKey("BlockPos")) return new Object[] {null};
		return new Object[] {
			Vector3.Vec3dToVector3(((BlockPos)context.extraContext.get("BlockPos")).toCenterPos())
		};
	}
}
